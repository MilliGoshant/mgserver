...
// HandlerFunc defines the handler used byserver middleware as return value.
...
type Engine struct {
	...
	// match those stored at `(*server.Engine).RemoteIPHeaders`. If no IP was
	// fetched, it falls back to the IP obtained from
	// `(*server.Context).Request.RemoteAddr`.
	ForwardedByClientIP bool

	// AppEngine was deprecated.
	// Deprecated: USE `TrustedPlatform` WITH VALUE `server.PlatformGoogleAppEngine` INSTEAD
	...
	// `(*server.Engine).ForwardedByClientIP` is `true` and
	// `(*server.Context).Request.RemoteAddr` is matched by at least one of the
	// network origins of list defined by `(*server.Engine).SetTrustedProxies()`.
	RemoteIPHeaders []string

	// TrustedPlatform if set to a constant of value server.Platform*, trusts the headers set by
...
// `(*server.Engine).ForwardedByClientIP` is `true`. `TrustedProxies`
...
func (engine *Engine) Run(addr ...string) error {
	...
	err := engine.RegisterETCD(address)
